...
#>
    ...
    PROCESS {
        ...
        if ($CloudEvent.DataContentType -eq $dataContentType -or `
            ($null -eq $CloudEvent.DataContentType -and # Datacontent Type is Optional, if it is not specified we assume it is JSON as per https://github.com/cloudevents/spec/blob/v1.0.1/spec.md#datacontenttype
...
#>
    ...
    PROCESS {
        ...
        $cloudEventFormatter = New-Object 'CloudNative.CloudEvents.NewtonsoftJson.JsonEventFormatter'
        ...
        # Build HTTP headers
        foreach ($attribute in $cloudEvent.GetPopulatedAttributes()) {
            $ceAttribute =  [CloudNative.CloudEvents.CloudEventsSpecVersion]::Default.AllAttributes | Where-Object { $_.Name -eq $attribute.Key }
            
            $headers.Add(($HttpHeaderPrefix + $attribute.Key), $ceAttribute.Format($attribute.Value))
        }

        # Add specversion attributes
        $headers.Add($SpecVersionHttpHeader1, $cloudEvent.SpecVersion.VersionId)
        $headers.Add($SpecVersionHttpHeader2, $cloudEvent.SpecVersion.VersionId)
        ...
        if ($ContentMode -eq [CloudNative.CloudEvents.ContentMode]::Structured) {
            try {
                ...
                # CloudEventFormatter is instance of 'CloudNative.CloudEvents.NewtonsoftJson.JsonEventFormatter' from the
                # .NET CloudEvents SDK for the purpose of fomatting structured mode
                $buffer = $cloudEventFormatter.EncodeStructuredModeMessage($cloudEvent, [ref] $contentType)
                $result | Add-Member -MemberType NoteProperty -Name 'Body' -Value $buffer
                $result.Headers.Add('Content-Type', $contentType)
            } catch {
                throw (New-Object Exception -ArgumentList 'Cannot convert structured mode cloud event. Only json content type is supported.', $_.Exception)
            }            
        }
        ...
        if ($ContentMode -eq [CloudNative.CloudEvents.ContentMode]::Binary) {
            ...
            if ($null -ne $cloudEvent.DataContentType) {
    ...
    PROCESS {
        ...
        if ($null -ne $Headers['Content-Type']) {
            ...
            if ($ContentType.StartsWith([CloudNative.CloudEvents.CloudEvent]::MediaType,
                ...
                if ($ctParts[0].Trim().StartsWith(([CloudNative.CloudEvents.CloudEvent]::MediaType) + '+json',
                        [StringComparison]::InvariantCultureIgnoreCase)) {

                    # Structured Mode supports non-batching JSON format only
                    # https://github.com/cloudevents/spec/blob/v1.0.1/http-protocol-binding.md#14-event-formats

                    # .NET SDK 'CloudNative.CloudEvents.NewtonsoftJson.JsonEventFormatter' type is used
                    ...
                    $formatter = New-Object 'CloudNative.CloudEvents.NewtonsoftJson.JsonEventFormatter'
                    $result = $formatter.DecodeStructuredModeMessage($Body,  $ContentType, $null)
                ...
                foreach ($httpHeader in $Headers.GetEnumerator()) {
                    ...
                    if ($httpHeader.Key.StartsWith($HttpHeaderPrefix, [StringComparison]::InvariantCultureIgnoreCase)) {
                        ...
                        if ($version -ne [CloudNative.CloudEvents.CloudEventsSpecVersion]::V0_1 -and `
                            ...
                            $jsonFormatter = New-Object 'CloudNative.CloudEvents.NewtonsoftJson.JsonEventFormatter'

                            $cloudEvent[$name] = $jsonFormatter.DecodeAttribute($version, $name,
                                [System.Text.Encoding]::UTF8.GetBytes($headerValue), $null);
                        }
                        else {
                            $ceAttribute =  [CloudNative.CloudEvents.CloudEventsSpecVersion]::Default.AllAttributes | Where-Object { $_.Name -eq $name }
                            if ($null -ne $ceAttribute) {
                                $cloudEvent[$ceAttribute.Name] = $ceAttribute.Parse($headerValue)
                            }                            
                        }
                    }
                }

                if ($null -ne $Headers['Content-Type'] -and $Headers['Content-Type'][0] -is [string]) {
